...
cities.developers <- cities.developers %>% group_by(place, region, internet.speed) %>% drop_na() %>% summarise(count = n())
#cities.developers["scale"] <- scale(cities.developers$count, center = FALSE)
cities.developers <- cities.developers %>% arrange(desc(count))
```

```{r}
ggplot(cities.developers, aes(x = internet.speed, y = count, fill = region)) + geom_bar(stat = "identity") + scale_x_log10()
```
