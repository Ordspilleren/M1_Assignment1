...
Let's also convert the population to a numeric value, more specifically a double like the above. The regular expression below takes everything that isn't a numeric value and replaces it with nothing.
...
Let's use the same trick for other numeric values that we want to use later.
...
```{r}
library(factoextra)
cities.clustering <- cities %>% mutate(walkability = as.numeric(walkability), quality.of.life = as.numeric(quality.of.life)) %>% drop_na()
km.cities <- kmeans(cities.clustering %>% select(walkability, quality.of.life, population) %>% scale(), centers = 3)
km.cities
fviz_cluster(km.cities, data = cities.clustering %>% select(walkability, quality.of.life, population) %>% scale())
```
